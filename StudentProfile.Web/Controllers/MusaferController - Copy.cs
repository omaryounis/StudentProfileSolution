...
{
    public partial class Musafer1Controller : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        request.Departing = DateTime.ParseExact(Departing, "MM/dd/yyyy", CultureInfo.InvariantCulture);
                        if (request.FlightsType == "R")
                        {
                            request.Returning = DateTime.ParseExact(Returning, "MM/dd/yyyy", CultureInfo.InvariantCulture);
                        }
        ...
        public ActionResult GetStudentAdvertisement()
        {
            ...
            if (StData != null)
            {
                ...
                var data = dbSC.TravelAdvertisement.Select(x => new
                {
                    ID = x.ID,
                    StudentId = studentid,
                    AdName = x.AdName,
                    ...
                    IsActive = x.IsActive,
                    TicketNumber = x.StTravelRequest.FirstOrDefault().StudentsTravelOrder.FirstOrDefault().TicketNumber,
                ...
                }).ToList().Where(m => m.NationalityID.Split(',').Any(p => p == NationalityID) && m.DegreeID.Split(',').Any(p => p == DegreeID) && m.StStatusID.Split(',').Any(p => p == StStatusID)).ToList();
        ...
        public ActionResult IsValidStudentData()
        {
            ...
            if (UniversityStudent != null)
            {
                ...
                if (UniversityStudent != null)
                {
                    ...
                    {
                        return Json(notify = new notify() { Message = "جارى مراجعه وتدقيق واعتماد الوثائق الخاصه بك", Type = "error", status = 500 }, JsonRequestBehavior.AllowGet);
                    }
        ...
        public ActionResult CheckStudentData(int id /*==>advertisementID*/)
        {
            try
            {
                ...
                if (UnStData != null)
                {
                    ...
                    foreach (var data in result.AsQueryable().ToList())
                    {
                        if (data > 0 && dbSC.TravelAdvertisement.Find(id).flightsType == "R")
        ...
        public ActionResult CheckStudentTicketAdvertisement(int id)
        {
            ...
            if (Check != null)
            {
                return Json(notify = new notify() { Message = "تم التقديم علي هذا الاعلان مسبقا يمكنك تعديل الرغبات", Type = "error", status = 500 }, JsonRequestBehavior.AllowGet);
            }
        ...
        public ActionResult GetStudentTravelRequestData(int ID)
        {
            ...
            var data = dbSC.StudentsTravelOrder.Where(x=>x.StTravelRequest.TravelAdvertisement.ID == ID && x.StudentID == StudentId).Select(x => new {
                Departing = x.StTravelRequest != null ? x.StTravelRequest.Departing.Day + "/" + x.StTravelRequest.Departing.Month + "/" + x.StTravelRequest.Departing.Year : null,
                Returning = x.StTravelRequest != null ? x.StTravelRequest.Returning.Value.Day + "/" + x.StTravelRequest.Returning.Value.Month + "/" + x.StTravelRequest.Returning.Value.Year : null,
                InsertDate = x.StTravelRequest != null ? x.StTravelRequest.InsertDate.Day + "/" + x.StTravelRequest.InsertDate.Month + "/" + x.StTravelRequest.InsertDate.Year : null,
                TravelOrder = x.TravelOrderID,
                AgentRefNumber = x.TravelOrders != null ? x.TravelOrders.AgentRefNumber : null,
                GivenAmount = x.GivenAmount,
                TicketNumber = x.TicketNumber == "[]" ? "" : x.TicketNumber,
                Tracking = x.StTravelRequest != null? x.StTravelRequest.TransportationTracking.Tracking: null
            }).ToList();
        ...
        public ActionResult GetTravelRequests(string TravelAdvertisementIds)
        {
            ...
            var data = dbSC.StTravelRequest.Where(x => travelAdvertisementIds.Any(p => p == x.AdvertisementID.ToString()))
            .Select(x => new
            {
                x.ID,
                x.AdvertisementID,
                ...
                InsertDate = x.InsertDate != null ? x.InsertDate.Day + "/" + x.InsertDate.Month + "/" + x.InsertDate.Year : null
