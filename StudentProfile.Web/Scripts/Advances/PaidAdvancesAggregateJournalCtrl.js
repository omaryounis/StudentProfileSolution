(function () {
    app.controller("PaidAdvancesAggregateJournalCtrl",
        ["$scope", "$http", "$timeout",
            function ($scope, $http, $timeout) {
                ...
                var DataSourcePaidDocAdvancesGrid = new DevExpress.data.DataSource({
                    ...
                    loadMode: "raw",
                    load: function () {
                        return $.getJSON("/Advances/GetDocsOfAdvances?type=A", function (data) {
                        });
                    }
                });
                /*********************************** Permissions *********************************/
                ...
                $http({
                    method: "Get",
                    url: "/Advances/GetAdvancePaymentPermissions?screenId=95"
                }).then(function (data) {
                    debugger;
            ...
            /*--------------------------------* Permissions *--------------------------------*/
                ...
                $scope.PaidDocAdvancesGrid = {
                    ...
                    onSelectionChanged: function (info) {
                        ...
                        //   // $scope.selectedRowsKeys.push(info.selectedRowKeys[i]);

                        //}
                        //console.log($scope.selectedRowsKeys)
                        //console.log($scope.selectedRowsKeys1)

                        $scope.selectedRowsKeys = info.selectedRowKeys.join(',');
                        $scope.PostMultiDocAdvancesbtnText = info.currentSelectedRowKeys.length > 1 ? "ترحيل القيد للسندات المحددة" : "ترحيل القيد للسند المحدد";
                        info.selectedRowKeys = [];
                    },
                    masterDetail: {
                        enabled: true,
