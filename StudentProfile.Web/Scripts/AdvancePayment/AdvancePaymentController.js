app.controller("AdvancePaymentCtrl", ["$scope", 'AdvancePaymentSrvc', '$http', function ($scope, AdvancePaymentSrvc, $http) {

    $scope.message = 'تم صرف السلف بنجاح';
    ...
    $http({
        method: "Get",
        url: "/AdvancePayment/GetAdvancePaymentPermissions?screenId=92"
    }).then(function (data) {
        debugger;
    ...
    $scope.gridAdvanceRequests = {
        ...
        onInitialized: function (e) {
            debugger;
            $scope.SelectedType = "A";
            $scope.MainTitle = "صرف السلف";
            $scope.BtnCachTitle = "صرف السلف المحددة";
            $scope.PopupCashTitle = "صرف السلف"; 
        }
    };
    ...
    $scope.btnCash = {
        ...
        onClick: function (e) {
            ...
            if ($scope.gridSelectedRowsData.length === 0) {
                DevExpress.ui.notify({ message: 'عفوا لم يتم تحديد أي سلف للصرف', type: 'error', displayTime: 3000, closeOnClick: true });
                return;
            } else {
    ...
    let DDL_StudentsDataSource = new DevExpress.data.DataSource({
        ...
        load: function () {
            debugger;
            return $.getJSON("/AdvancePayment/GetAllStudentsHaveAdvances?Type=A", function (data) {
            });
        }
    });
        ...
        AdvancePaymentSrvc.SaveAdvances({ AccountId: $scope.AccountId, AdvanceRequests: $scope.gridSelectedRowsData, Notes: $scope.Note, PostJournal: $scope.JournalentrycheckValue, PaymentDate: $scope.JournalentryDateValue })
            .then(function (data) {
                if (data.data.status === 200) {
                    DevExpress.ui.notify({ message: "تم صرف السلف بنجاح", type: data.data.Type, displayTime: 3000, closeOnClick: true });
